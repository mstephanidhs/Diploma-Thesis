...
if __name__ == "__main__":
    try:
        ...
        log_file = os.path.join(script_dir, 'encryption', 'logs', 'run.log')
        logging.basicConfig(filename=log_file, level=logging.INFO, format='%(asctime)s - %(message)s')
        
        # Navigate to the directory containg the watch.py script
        python_script_path = os.path.join(script_dir, 'encryption')

        # Start Python script in another command prompt window
        python_script_process = subprocess.Popen(["start", "cmd", "/k", "python watch.py"], cwd=python_script_path,shell=True)
        ...
        logging.info("Slarchive Container started.")
